...
void solve_1_enable(PetriNet N,vector<string> Trans, string tranEnabled, vector<string>sequence, vector<map<string, int>> &marking){
    ...
    if (transEnabled.size() == 1) {
        ...
        solve_2_enables(N, Trans, transEnabled, sequence, marking);
    } 
    else {
        return;
    }
...
void item4() {
    ...
    vector<map<string, int>> marking;
    N.setMarking(); 
    marking.push_back(N.getMarking());
    cout << endl;
    vector<string> Trans{ "start", "change", "end" }; 
    solve_1_enable(N, Trans, "start", sequence, marking);
    cout << marking.size();
    N.cleanUp(); 
}
